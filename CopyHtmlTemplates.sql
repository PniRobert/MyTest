...
SET @sourceDB = 'HTMLTemplate_Apollo_Green'
SET @targetDB = 'HTMLTemplate_Apollo_Dev'
...
BEGIN
    SET @sqlStatement = 'DELETE FROM [' + @targetDB + '].dbo.[HtmlTemplateVersion]'
    EXEC sp_executesql @sqlStatement
    SET @sqlStatement = 'DELETE FROM [' + @targetDB + '].dbo.[HtmlTemplate]'
    EXEC sp_executesql @sqlStatement
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @sqlStatement = 'SELECT @tempId=htmlTemplateID FROM [' + @targetDB + ']..[HtmlTemplate] WITH(NOLOCK) WHERE siteID=' + CAST(@targetSiteID as varchar) + ' AND templateKey=''' + @templateKey + ''''
            EXEC sp_executesql @sqlStatement, N'@tempId int OUTPUT', @tempId=@targetTemplateId OUTPUT
            IF @targetTemplateId IS NULL
            BEGIN
	            SET @sqlStatement = 'INSERT INTO [' + @targetDB + ']..[HtmlTemplate](uniqueID, siteID, templateKey) VALUES(NEWID(),' + CAST(@targetSiteID as varchar) + ', ''' + @templateKey  + ''')'
	            EXEC sp_executesql @sqlStatement
                SET @sqlStatement = 'SELECT @tempId=htmlTemplateID FROM [' + @targetDB + ']..[HtmlTemplate] WITH(NOLOCK) WHERE siteID=' + CAST(@targetSiteID as varchar) + ' AND templateKey=''' + @templateKey + ''''
                EXEC sp_executesql @sqlStatement, N'@tempId int OUTPUT', @tempId=@targetTemplateId OUTPUT
            END
            ELSE
            BEGIN
                SET @sqlStatement =  'SELECT @vNum=MAX(versionNumber) FROM [' + @targetDB + ']..[HtmlTemplateVersion] WITH(NOLOCK) WHERE htmlTemplateID=' + CAST(@targetTemplateId as varchar)
            ...
            SET @sqlStatement = 'INSERT INTO [' + @targetDB + ']..[HtmlTemplateVersion](htmlTemplateID, culture, versionNumber, templateTypeID, publishStartDate, publishState, templateData)'
