...
SET @sourceDB = 'HTMLTemplate_Apollo_Blue'
SET @targetDB = 'HTMLTemplate_WIP4'

SET @sourceSiteId=NULL
SET @targetSiteID=NULL
SET @sqlStatement = 'SELECT TOP 1 @id=siteID FROM [' + @sourceDB + '].dbo.[Site]  WITH(NOLOCK) WHERE siteCode=@cd'
EXEC sp_executesql @sqlStatement, N'@cd varchar(20), @id int OUTPUT', @cd=@siteCode, @id=@sourceSiteId OUTPUT
SET @sqlStatement = 'SELECT TOP 1 @id=siteID FROM [10.211.10.155\TOSQLSTG013].[' + @targetDB + '].dbo.[Site]  WITH(NOLOCK) WHERE siteCode=@cd'
...
BEGIN
    SET @templateCursorStatement = 'DECLARE tmpCursor CURSOR FOR SELECT htmlTemplateID, templateKey FROM [' + @sourceDB + ']..[HtmlTemplate]  WITH(NOLOCK) WHERE siteID = ' + CAST(@sourceSiteId as varchar)
    ...
    BEGIN
        ...
        BEGIN
            SET @sqlStatement = 'SELECT TOP(1) @cu=culture, @tpId=templateTypeID, @data=templateData FROM ['+ @sourceDB + ']..[HtmlTemplateVersion]  WITH(NOLOCK) WHERE htmlTemplateID=' + CAST(@htmlTemplateId as varchar) + ' AND publishState=''Published'' ORDER BY versionNumber DESC'
            ...
            SET @sqlStatement = 'SELECT @tempId=htmlTemplateID FROM [10.211.10.155\TOSQLSTG013].[' + @targetDB + '].dbo.[HtmlTemplate] WITH(NOLOCK) WHERE siteID=' + CAST(@targetSiteID as varchar) + ' AND templateKey=''' + @templateKey + ''''
            EXEC sp_executesql @sqlStatement, N'@tempId int OUTPUT', @tempId=@targetTemplateId OUTPUT
            IF @targetTemplateId IS NULL
            BEGIN
	            SET @sqlStatement = 'INSERT INTO [10.211.10.155\TOSQLSTG013].[' + @targetDB + '].dbo.[HtmlTemplate](uniqueID, siteID, templateKey) VALUES(NEWID(),' + CAST(@targetSiteID as varchar) + ', ''' + @templateKey  + ''')'
            ...
            ELSE
            BEGIN
                SET @sqlStatement =  'SELECT @vNum=MAX(versionNumber) FROM [10.211.10.155\TOSQLSTG013].[' + @targetDB + '].dbo.[HtmlTemplateVersion] WITH(NOLOCK) WHERE htmlTemplateID=' + CAST(@targetTemplateId as varchar)
            ...
            SET @sqlStatement = 'INSERT INTO [10.211.10.155\TOSQLSTG013].[' + @targetDB + '].dbo.[HtmlTemplateVersion](htmlTemplateID, culture, versionNumber, templateTypeID, publishStartDate, publishState, templateData)'
