...
SET @sourceDB = 'PROD_StaplesUS_HTMLTemplate_green'
SET @targetDB = 'HTMLTemplate_Apollo_Green'

SET @sourceSiteId=NULL
SET @targetSiteID=NULL
SET @sqlStatement = 'SELECT TOP 1 @id=siteID FROM [10.211.10.124\SQL2014].[' + @sourceDB + '].dbo.[Site]  WITH(NOLOCK) WHERE siteCode=@cd'
...
BEGIN
    ...
    EXEC sp_executesql @sqlStatement
    SET @templateCursorStatement = 'DECLARE tmpCursor CURSOR FOR SELECT htmlTemplateID, templateKey FROM [10.211.10.124\SQL2014].[' + @sourceDB + '].dbo.[HtmlTemplate]  WITH(NOLOCK) WHERE siteID = ' + CAST(@sourceSiteId as varchar)
    ...
    BEGIN
        ...
        BEGIN
            SET @sqlStatement = 'SELECT TOP(1) @cu=culture, @tpId=templateTypeID, @data=templateData FROM [10.211.10.124\SQL2014].['+ @sourceDB + '].dbo.[HtmlTemplateVersion]  WITH(NOLOCK) WHERE htmlTemplateID=' + CAST(@htmlTemplateId as varchar) + ' AND publishState=''Published'' ORDER BY versionNumber DESC'
